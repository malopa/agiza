...
export default function page() {
    ...
    let token = getToken();
    const {data:new_orders,isLoading:isNewOrderLoading} = useQuery({queryKey:['returned-orders'],queryFn:async ()=> await getNewOrderByStatus({token,status:'returned'}) })
    const {data:group_orders} = useQuery({queryKey:['returned-group-orders'],queryFn:async ()=> await getClientGroupOrder({token,status:'returned'}) })
  ...
  // end table search 
   ...
   const columns = [
     ...
     },
     {
       ...
       dataIndex: 'status',
       key:'status',
       render:(text,record)=><span className='capitalize'><Tag color="green">{record?.status}</Tag></span>
     },
     {
       title: 'Supplier',
       dataIndex: 'supplier',
   ...
   
     {
       ...
       render: (text,record) => <div>
                           <span className='mx-1 cursor-pointer' onClick={()=>{setVisibleSupplier(true);setActiveGroup(record);}}><Tooltip title="Assign to Staff Member"><ExportOutlined /></Tooltip></span>
  ...
  return (
    <div className='min-h-[70vh]'>
        <div className='flex w-full justify-between  items-center'>
            <div className='uppercase'>Retuned Orders</div>
      </div>
